...
class BiLSTMForecaster(nn.Module):
    """
    Bidirectional LSTM solar flare forecaster with MC Dropout uncertainty estimation
    ...
    def predict_with_uncertainty(self, x: torch.Tensor, n_samples: int = 20) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            mean_probs: [batch_size, num_horizons, num_classes]
            mean_reg: [batch_size, num_horizons, 1]
            std_reg: [batch_size, num_horizons, 1]
        """
        ...
        stacked_probs = torch.cat(all_probs, dim=0) # [samples, batch, horizons, classes]
        stacked_regs = torch.cat(all_regs, dim=0)   # [samples, batch, horizons, 1]
