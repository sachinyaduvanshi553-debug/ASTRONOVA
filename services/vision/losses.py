...
class SolarVisionLoss(nn.Module):
    ...
    def forward(self, predictions: dict, targets: dict) -> tuple[torch.Tensor, dict]:
        ...
        flare_class = targets.get(
            "flare_class", torch.zeros(pred_img.shape[0], dtype=torch.long, device=pred_img.device)
        )
        log_flux = targets.get(
            "log_flux",
            torch.zeros((pred_img.shape[0], 1), dtype=torch.float32, device=pred_img.device),
        )
        ...
        if target_img is not None and pred_img is not None:
            ...
            total_loss += (
                self.weights["mse"] * l_mse
                + self.weights["l1"] * l_l1
                + self.weights["ssim"] * l_ssim
                + self.weights["perceptual"] * l_perceptual
