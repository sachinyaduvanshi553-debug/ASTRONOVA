...
from .encoder import ImageEncoder, PhysicsEncoder, TemporalEncoder
...
class SolarVisionPredictor(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(spatial_dim, 128), nn.ReLU(), nn.Dropout(p=dropout), nn.Linear(128, n_classes)
        )

        # Regression head — log10(flux)
        self.regressor = nn.Sequential(nn.Linear(spatial_dim, 64), nn.ReLU(), nn.Dropout(p=dropout), nn.Linear(64, 1))
