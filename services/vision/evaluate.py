...
def _fspecial_gaussian(
    size: int = 11, sigma: float = 1.5, channels: int = 1, device: torch.device | None = None
) -> torch.Tensor:
...
def calculate_fid_features(images: torch.Tensor, device: torch.device | None = None) -> torch.Tensor:
