...
class XAIVisualizer:
    ...
    def generate_uncertainty_map(self, image_sequence, telemetry, physics, num_samples=10):
        ...
        spatial_uncertainty = (spatial_uncertainty - np.min(spatial_uncertainty)) / (
            np.max(spatial_uncertainty) - np.min(spatial_uncertainty) + 1e-8
        )
    ...
    def temporal_importance(
        ...
        with torch.no_grad():
            ...
            base_pred = base_out["predicted_image"] if isinstance(base_out, dict) else base_out
        ...
        for t in range(T):
            ...
            with torch.no_grad():
                ...
                occ_pred = occ_out["predicted_image"] if isinstance(occ_out, dict) else occ_out
